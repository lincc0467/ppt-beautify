...
// Helper: file:// URL → 本機路徑（Windows 上要去掉 /D:/ 的前導斜線並解碼中文檔名）
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (el.tagName === 'DIV' && el.dataset && el.dataset.pptxMerge === 'true') {
        ...
        const baseStyle = {
          ...
          charSpacing: (firstComputed.letterSpacing && firstComputed.letterSpacing !== 'normal')
            ? pxToPoints(firstComputed.letterSpacing) : null,
      ...
      const baseStyle = {
        ...
        color: rgbToHex(computed.color),
        align: computed.textAlign === 'start' ? 'left' : computed.textAlign,
        // letter-spacing → charSpacing（pptxgenjs 支援，原腳本漏接，字距全丟失）
        charSpacing: (computed.letterSpacing && computed.letterSpacing !== 'normal')
          ? pxToPoints(computed.letterSpacing) : null,
